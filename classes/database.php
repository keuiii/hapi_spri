...
public function loginCustomerAuth($username, $password) {
    if (empty($username) || empty($password)) {
        return ["success" => false, "message" => "Please fill in all fields."];
    }

    try {
        // Check for customer account - support both username and email
        $user = $this->fetch(
        ...
        if (!$user) {
            ...
            $anyUser = $this->fetch("SELECT role FROM users WHERE (username = ? OR email = ?) LIMIT 1", [$username, $username]);
            if ($anyUser) {
                return ["success" => false, "message" => "This account is not a customer account."];
            }
            return ["success" => false, "message" => "Account not found. Please check your username or register first."];
        }
...
public function registerCustomer($firstname, $lastname, $username, $password) {
    // Validate inputs
    ...
    $existing = $this->fetch("SELECT id FROM users WHERE username = ? LIMIT 1", [$username]);
    if ($existing) {
    ...
    try {
        ...
        $success = $this->insert(
            "INSERT INTO users (firstname, lastname, username, password, role, created_at) VALUES (?, ?, ?, ?, 'customer', NOW())",
