...
<div id="qvBackdrop" class="qv-backdrop" hidden>
  <div class="qv-modal" role="dialog" aria-modal="true" aria-labelledby="qvName">
    ...
    <div class="qv-body">
      ...
      <p id="qvDesc" class="qv-desc"></p> <!-- 🔥 Added -->
...
(function () {
  ...
  if (!backdrop || !imgEl || !nameEl || !priceEl || !descEl || !addBtn || !viewBtn) return;
  ...
  function openQV(data) {
    ...
    viewBtn.href = `view_product.php?id=${encodeURIComponent(data.id)}`;
  ...
  // Open Quick View (with fetch from PHP)
  ...
  document.addEventListener('keydown', (e) => {
    if (!backdrop.hidden && e.key === 'Escape') closeQV();
  });
  ...
  addBtn.addEventListener('click', () => {
    ...
    .then(tx => {
      if (tx.trim() === 'success') {
        ...
        console.log('Cart server response:', tx);
